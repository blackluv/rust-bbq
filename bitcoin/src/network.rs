...
mod tests {
    ...
    #[test]
    fn serialize_test() {
        assert_eq!(serialize(&Network::Bitcoin.magic()), &[0xfe, 0xe4, 0xd9, 0x42]);
        assert_eq!(serialize(&Network::Testnet.magic()), &[0xfe, 0xe4, 0xd9, 0x42]);
        ...
        assert_eq!(deserialize(&[0xfe, 0xe4, 0xd9, 0x42]).ok(), Some(Network::Bitcoin.magic()));
